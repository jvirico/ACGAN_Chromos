...
def generator2(checkpoints_path='checkpoints/G_200.pt', num_generations = 1, label = 1, split = False):
    ...
    if(num_generations == 0):
        while True:
            sample_image(1, 'syn_'+ str(label) + '_' + ckp_name + '_all',label-1)
    else:
        if(not split):
            sample_image(num_generations, 'syn_'+ str(label) + '_' + ckp_name,label-1)
        else:
            for i in range(0 , num_generations):
                sample_image(1, 'syn_'+ str(label) + '_' + ckp_name +'_'+ str(i+1),label-1)